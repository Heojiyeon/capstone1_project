...
async function tour_run() {
  try {
    ...
    const t_recommend = await tourplace_recommend.tourplace_recommend(
      connection,
      f1,
      want_theme,
      disabled_type
    ...
    return [u, for_front, t_recommend];
  } catch (err) {
    console.log(err);
  } finally {
    if (connection) {
...
async function res_run(user_choice_tid) {
  try {
    ...
    const f2 = await common.filter_address2(
      connection,
      choice_add2,
      "restaurant"
    );
    const r_recommend = await restaurant_recommend.restaurant_recommend(
      connection,
      f2,
      user_choice_tid,
      disabled_type
...
async function hotel_run(user_choice_tid) {
  try {
    ...
    const h_recommend = await hotel_recommend.hotel_recommend(
      connection,
      f3,
      user_choice_tid,
      disabled_type
...
// 최종 추천 코스 정리 (음식점, 숙박 X)
async function uc_run3(tid) {
  try {
    await db_run();
    // 사용자에게 보여줄 수 있는 오브젝트를 담은 배열 생성
    uc_for_front = [];
    const ucti = await topten.load_info(connection, tid);
    uc_for_front.push(ucti);
    console.log(uc_for_front);
